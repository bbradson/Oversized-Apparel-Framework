#if false
//Copy of vanilla for easier transpiling. DrawHeadHair.DrawApparel and DrawBodyApparel show the result after applying transpilers.
...
public class PawnRenderer
{
	...
	public void RenderPawnAt(Vector3 drawLoc, Rot4? rotOverride = null, bool neverAimWeapon = false)
	{
		...
		if (pawn.GetPosture() == PawnPosture.Standing)
		{
			...
			{
				RenderPawnInternal(drawLoc, 0f, renderBody: true, rot, curRotDrawMode, pawnRenderFlags);
			}
	...
	private void DrawHeadHair(Vector3 rootLoc, Vector3 headOffset, float angle, Rot4 bodyFacing, Rot4 headFacing, RotDrawMode bodyDrawType, PawnRenderFlags flags)
	{
		...
		{
			Mesh mesh3 = OversizedApparel.OversizedApparel.ChangeMeshSize(graphics.HairMeshSet.MeshAt(headFacing), headFacing, apparelRecord);
			...
			else
			{
				...
				Vector3 loc3 = rootLoc + headOffset + OversizedApparel.OversizedApparel.GetDrawOffset(apparelRecord, headFacing);
	...
	public void DrawBodyApparel(Vector3 shellLoc, Vector3 utilityLoc, Mesh bodyMesh, float angle, Rot4 bodyFacing, PawnRenderFlags flags)
	{
		List<ApparelGraphicRecord> apparelGraphics = graphics.apparelGraphics;
		Quaternion quaternion = Quaternion.AngleAxis(angle, Vector3.up);
		for (int i = 0; i < apparelGraphics.Count; i++)
		{
			...
			if (apparelGraphicRecord.sourceApparel.def.apparel.LastLayer == ApparelLayerDefOf.Shell && !apparelGraphicRecord.sourceApparel.def.apparel.shellRenderedBehindHead)
			{
				...
				Vector3 loc = shellLoc + OversizedApparel.OversizedApparel.GetDrawOffset(apparelGraphicRecord, bodyFacing);
				if (apparelGraphicRecord.sourceApparel.def.apparel.shellCoversHead)
				{
					loc.y += 0.00289575267f;
				}
				GenDraw.DrawMeshNowOrLater(OversizedApparel.OversizedApparel.ChangeMeshSize(bodyMesh, bodyFacing, apparelGraphicRecord), loc, quaternion, material, flags.FlagSet(PawnRenderFlags.DrawNow));
			}
			if (RenderAsPack(apparelGraphicRecord.sourceApparel))
			{
				...
				if (apparelGraphicRecord.sourceApparel.def.apparel.wornGraphicData != null)
				{
					...
					GenDraw.DrawMeshNowOrLater(OversizedApparel.OversizedApparel.ChangeMeshSize(bodyMesh, bodyFacing, apparelGraphicRecord), matrix, material2, flags.FlagSet(PawnRenderFlags.DrawNow));
				}
				else
				{
					GenDraw.DrawMeshNowOrLater(OversizedApparel.OversizedApparel.ChangeMeshSize(bodyMesh, bodyFacing, apparelGraphicRecord), shellLoc, quaternion, material2, flags.FlagSet(PawnRenderFlags.DrawNow));
				}
			}
		}
	}
