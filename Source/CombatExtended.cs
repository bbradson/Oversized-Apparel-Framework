	...
	public static class Harmony_PawnRenderer_DrawHeadHair
	{
		...
		private static void DrawHeadApparel(PawnRenderer renderer, Pawn pawn, Vector3 rootLoc, Vector3 headLoc, Vector3 headOffset, Rot4 bodyFacing, Quaternion quaternion, PawnRenderFlags flags, Rot4 headFacing, ref bool hideHair)
		{
			...
			for (int i = 0; i < apparelGraphics.Count; i++)
			{
				...
				else if (apparelRecord.sourceApparel.def.apparel.LastLayer == ApparelLayerDefOf.Belt/*apparelRecord.sourceApparel.def.apparel.LastLayer.GetModExtension<ApparelLayerExtension>()?.IsHeadwear ?? false*/)
				{
					...
					if (apparelRecord.sourceApparel.def.apparel.hatRenderedFrontOfFace)
					{
						...
						matrix.SetTRS(maskLoc + OversizedApparel.OversizedApparel.GetDrawOffset(apparelRecord, headFacing), quaternion, customScale);
						GenDraw.DrawMeshNowOrLater(OversizedApparel.OversizedApparel.ChangeMeshSize(mesh, headFacing, apparelRecord), matrix, apparelMat, flags.FlagSet(PawnRenderFlags.DrawNow));
					}
					else
					{
						Matrix4x4 matrix = new Matrix4x4();
...
//							hideHair = apparelRecord.sourceApparel?.def?.GetModExtension<ApperalRenderingExtension>()?.HideHair ?? true;
						...
						matrix.SetTRS(headwearPos + OversizedApparel.OversizedApparel.GetDrawOffset(apparelRecord, headFacing), quaternion, customScale);
						GenDraw.DrawMeshNowOrLater(OversizedApparel.OversizedApparel.ChangeMeshSize(mesh, headFacing, apparelRecord), matrix, apparelMat, flags.FlagSet(PawnRenderFlags.DrawNow));
					}
				}
			}
